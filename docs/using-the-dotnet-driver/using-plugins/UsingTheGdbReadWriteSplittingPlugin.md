...
|------------------------------------|:-------:|:---------------------------------------------------------------------------------------------------------------------------------:|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:-------------:|
| `GlobalClusterInstanceHostPatterns` | String  | When connecting through a Global Database endpoint (or any endpoint that cannot be mapped to per-region instance endpoints): Yes<br><br>Otherwise: No | Comma-separated list of per-region instance host patterns used to construct instance endpoints during topology discovery. Use `?` as the placeholder for the instance name, and include one pattern per region of the Global Database.<br><br>Example: `?.XYZ1.us-east-2.rds.amazonaws.com,?.XYZ2.us-west-2.rds.amazonaws.com`. See [Aurora Global Databases](../GlobalDatabases.md#global-cluster-instance-host-patterns). | `null` |
...
See the [PGReadWriteSplitting.cs](../../examples/AwsWrapperDataProviderExample/PGReadWriteSplitting.cs) example for the read/write splitting pattern. To adapt it for a Global Database, replace `readWriteSplitting` with `gdbReadWriteSplitting` in the `Plugins` parameter, set `GlobalClusterInstanceHostPatterns` (required when connecting through a Global Database endpoint), and set `GdbRwHomeRegion` (and the restriction parameters) as needed.
