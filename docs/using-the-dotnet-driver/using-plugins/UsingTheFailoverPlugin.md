...
| `TelemetryFailoverAdditionalTopTrace` | Boolean | No | Allows the wrapper to produce an additional telemetry span associated with failover. Such a span helps to facilitate telemetry analysis in AWS CloudWatch. Only applies when telemetry is enabled; see [Telemetry](../Telemetry.md) for the underlying setup. | `false` |

The cluster topology monitor may create new monitoring connections to query for DB cluster topology. You can configure these connections with driver-specific configurations by adding the `topology-monitoring-` prefix to the configuration parameters, as in the following example:
...
AwsWrapperConnection<NpgsqlConnection> connection = new(
        ...
        Password=pwd;
        Plugins=efm;
        topology-monitoring-commandTimeout=50;"
);
```
...
When telemetry is enabled (see [Telemetry](../Telemetry.md)), the Failover Plugin emits the following counters and spans:

- Counters: `writerFailover.triggered.count`, `writerFailover.completed.success.count`, `writerFailover.completed.failed.count`, `readerFailover.triggered.count`, `readerFailover.completed.success.count`, `readerFailover.completed.failed.count`. The full list with descriptions is in the [Telemetry metrics table](../Telemetry.md#failover-plugin).
- Spans: `failover to writer` and `failover to reader`, opened around the writer and reader failover operations respectively.
