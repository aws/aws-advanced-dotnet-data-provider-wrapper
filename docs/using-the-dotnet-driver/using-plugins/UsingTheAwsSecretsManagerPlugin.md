...
A Secrets Manager secret may be rotated, changing the cached credentials. Because the community drivers key their internal connection pool on the connection string, the wrapper supplies the rotated **password** to the target driver through its native password-provider mechanism (`NpgsqlDataSourceBuilder.UsePasswordProvider` for `Npgsql`, `MySqlConnection.ProvidePasswordCallback` for `MySqlConnector`) so the password is kept out of the connection string. With these drivers, **password** rotation does not fragment the connection pool.
