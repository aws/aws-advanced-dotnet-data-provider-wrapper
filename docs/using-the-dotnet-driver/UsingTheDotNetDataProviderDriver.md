...
These parameters are applicable to any instance of the AWS Advanced .NET Data Provider Wrapper. Parameters that are specific to a plugin are documented on that plugin's own page, linked from the [list of available plugins](#list-of-available-plugins).

| Parameter                       | Description                                                                                                                                                                                                                                                                                                                                                                                | Required                                                | Default Value |
|---------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------|---------------|
| `TargetConnectionType`          | The fully qualified type name of the underlying provider connection the wrapper should target (for example `MySqlConnector.MySqlConnection` or `Npgsql.NpgsqlConnection`).                                                                                                                                                                                                                  | False                                                   | `null`        |
| `TargetCommandType`             | The fully qualified type name of the underlying provider command. Usually inferred from the connection type and only needed when overriding the default.                                                                                                                                                                                                                                   | False                                                   | `null`        |
| `CustomDialect`                 | The `AssemblyQualifiedName` of a custom class implementing `IDialect`. When unset, the dialect is detected automatically.                                                                                                                                                                                                                                                                  | False                                                   | `null`        |
| `CustomTargetConnectionDialect` | The `AssemblyQualifiedName` of a custom class implementing `ITargetConnectionDialect`. When unset, the target connection dialect is detected automatically.                                                                                                                                                                                                                                 | False                                                   | `null`        |
| `SingleWriterConnectionString`  | Set to `true` when providing a connection string with multiple comma-delimited hosts where the cluster has only one writer. The writer must be the first host in the connection string.                                                                                                                                                                                                     | False                                                   | `false`       |
| `ClusterId`                     | A unique identifier for the cluster. Connections with the same cluster id share a cluster topology cache. This parameter is optional and defaults to `1`. When supporting multiple database clusters, this parameter becomes mandatory. Each connection string must include the `ClusterId` parameter with a value that can be any number or string. However, all connection strings associated with the same database cluster must use identical `ClusterId` values, while connection strings belonging to different database clusters must specify distinct values. Examples of value: `1`, `2`, `1234`, `abc-1`, `abc-2`. | If using multiple database clusters, yes; otherwise, no | `1`           |

### Application-wide Configuration

Some behavior is configured once per process rather than per connection, and is read from environment variables. These settings apply to every connection created by the application.

When identifying which cluster instance a connection points to, the wrapper can cache the identification result keyed by the connection host name. This avoids repeatedly querying the database to identify the same static host (such as a custom domain name or IP address). The cache is shared across all connections in the process, so it is governed by application-wide settings rather than connection-string parameters.

| Environment Variable             | Description                                                                                                                                                                                            | Default Value         |
|----------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------|
| `AWS_DOTNET_HOST_CACHE_ENABLED`  | Enables caching of host identification results by the connection host name. When enabled, subsequent identification calls for the same host use the cached result instead of querying the database again. | `true`                |
| `AWS_DOTNET_HOST_CACHE_REGEXP`   | A regular expression controlling which host names are eligible for host identification caching. Only hosts whose names match this expression will have their identification results cached or retrieved from the cache. | `.*` (any host name) |
...
| [Aurora Connection Tracker Plugin](./using-plugins/UsingTheAuroraConnectionTrackerPlugin.md)            | `auroraConnectionTracker` | Aurora                 | Tracks all the opened connections. In the event of a cluster failover, the plugin will close all the impacted connections. This plugin is enabled by default.                                                | None                                                                                    |
...
| [Blue/Green Deployment Plugin](./using-plugins/UsingTheBlueGreenPlugin.md)                             | `bg`                | Aurora, RDS            | Adds support for Blue/Green Deployments, coordinating connection routing between blue and green hosts during a switchover to minimize disruption.                                                            | None                                                                                    |
| [Limitless Connection Plugin](./using-plugins/UsingTheLimitlessConnectionPlugin.md)                    | `limitless`         | Aurora                 | Enables support for Amazon Aurora Limitless Database by balancing connections across the available transaction routers.                                                                                      | None                                                                                    |
| Aurora Global Database Failover Plugin                                                                  | `gdbFailover`       | Aurora Global Database | Extends failover functionality for Amazon Aurora Global Databases, supporting cross-region failover and an application home region.                                                                          | None                                                                                    |
| Aurora Global Database Read/Write Splitting Plugin                                                      | `gdbReadWriteSplitting` | Aurora Global Database | Switches between writer and reader instances for Amazon Aurora Global Databases, with home-region restrictions and optional global write forwarding.                                                     | None                                                                                    |
| Execution Time Connection Plugin                                                                       | `executionTime`     | Any database           | Logs the time taken to execute any .NET method.                                                                                                                                                              | None                                                                                    |
| Connect Time Connection Plugin                                                                         | `connectTime`       | Any database           | Logs the time taken to establish a connection.                                                                                                                                                               | None                                                                                    |
