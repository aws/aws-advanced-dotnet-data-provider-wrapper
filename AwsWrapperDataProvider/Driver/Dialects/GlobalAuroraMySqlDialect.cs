...
public class GlobalAuroraMySqlDialect : AuroraMySqlDialect, IGlobalAuroraTopologyDialect
{
    private static readonly ILogger<GlobalAuroraMySqlDialect> Logger = LoggerUtils.GetLogger<GlobalAuroraMySqlDialect>();

    internal static readonly string GlobalStatusTableExistsQuery =
        "SELECT 1 AS tmp FROM information_schema.tables WHERE"
        + " upper(table_schema) = 'INFORMATION_SCHEMA' AND upper(table_name) = 'AURORA_GLOBAL_DB_STATUS'";

    internal static readonly string GlobalInstanceStatusExistsQuery =
        "SELECT 1 AS tmp FROM information_schema.tables WHERE"
    ...
    public string RegionByInstanceIdQuery =>
        "SELECT AWS_REGION FROM information_schema.aurora_global_db_instance_status WHERE SERVER_ID = '{0}'";
    ...
    public override async Task<bool> IsDialect(DbConnection connection)
    {
        if (!await DialectUtils.CheckExistenceQueries(
                ...
                Logger,
                GlobalStatusTableExistsQuery,
                GlobalInstanceStatusExistsQuery))
        {
            return false;
        }
        ...
        {
            Logger.LogTrace(ex, Resources.Error_CantCheckDialect, nameof(GlobalAuroraMySqlDialect));
        }
