...
{
    private static readonly string HOSTS_SEPARATOR = ",";
    private static readonly string HOST_PORT_SEPARATOR = ":";
    ...
    public static IList<HostSpec> GetHostsFromProperties(Dictionary<string, string> props, HostSpecBuilder hostSpecBuilder)
    {
        ...
        IList<string> hostStringList =
            hostsString.Split(HOSTS_SEPARATOR, StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries);
        int port = PropertyDefinition.Port.GetInt(props) ?? HostSpec.NoPort;

        foreach (string hostPortString in hostStringList)
        {
            IList<string> hostPortPair = hostPortString.Split(HOST_PORT_SEPARATOR,
                2,
