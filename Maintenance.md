...
| August 4, 2026   | [Release 2.2.0](https://github.com/aws/aws-advanced-dotnet-data-provider-wrapper/releases/tag/2.2.0) |
| September 24, 2026 | [Release 3.0.0](https://github.com/aws/aws-advanced-dotnet-data-provider-wrapper/releases/tag/3.0.0) |
...
For minor version releases, `aws-advanced-dotnet-data-provider-wrapper` uses a “release-train” model. Approximately every four weeks we
release a new minor version which includes all the new features and fixes that are ready to go.
Having a set release schedule makes sure `aws-advanced-dotnet-data-provider-wrapper` is released in a predictable way and prevents a
backlog of unreleased changes.

In contrast, `aws-advanced-dotnet-data-provider-wrapper` releases new major versions only when there are a critical mass of
breaking changes (e.g. changes that are incompatible with existing APIs). This tends to happen if we need to
change the way the wrapper is currently working. Fortunately, the API is fairly mature and has not changed, however
in the event that the API changes we will release a version to be compatible. In the event that the API changes we will
release a version to be compatible.
...
and log all changes in the [changelog](./CHANGELOG.md).

# Maintenance Policy

For `aws-advanced-dotnet-data-provider-wrapper` new features and active development always takes place against the newest version.
The `aws-advanced-dotnet-data-provider-wrapper` project follows the semantic versioning specification for assigning version numbers
...
the major version number (e.g., increment from `aws-advanced-dotnet-data-provider-wrapper` 1.1.1 to `aws-advanced-dotnet-data-provider-wrapper` 2.0.0).
...
| 2             | 2.2.0                | Maintenance | Jun 16, 2026    | Sep 24, 2026             | Sep 24, 2027           |
| 3             | 3.0.0                | Current     | Sep 24, 2026    | N/A                      | N/A                    |
