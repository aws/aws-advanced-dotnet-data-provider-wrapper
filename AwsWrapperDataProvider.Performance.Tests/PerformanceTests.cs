...
public class PerformanceTests
{
    ...
    public async Task FailureDetectionTimeTest_EnhancedMonitoringEnabled(string efmPluginCode)
    {
        ...
        {
            SheetsWriter.WritePerformanceDataToFile(@$"./PerformanceResults-{DateTime.Now.ToString("yyyyMMdd-HHmmss")}.xlsx", EfmPerformanceDataList);
            EfmPerformanceDataList.Clear();
        }
    }
    ...
    public async Task FailureDetectionTimeTest_FailoverAndEnhancedMonitoringEnabled()
    {
        ...
        {
            SheetsWriter.WritePerformanceDataToFile($@"./FailoverAndEfmPerformanceResults-{DateTime.Now.ToString("yyyyMMdd-HHmmss")}.xlsx", FailoverAndEfmPerformanceDataList);
        ...
        int sleepDelayMs)
    {
        ...
        connectionString += $"Pooling=false;";
        connectionString += $"monitoring-CommandTimeout=1;";
        connectionString += $"monitoring-Timeout=1;";
        connectionString += $"ClusterTopologyRefreshRateMs=1000;";
        ...
        string instanceId)
    {
        ...
        for (int i = 0; i < repeatTimes; i++)
        {
            ...
            try
            {
                ...
                Task.Run(async () =>
                {
                    try
                    {
                        ...
                        Console.WriteLine($@"Starting stopwatch {(DateTime.UtcNow.ToLongTimeString())}");
                        stopwatch.Start();
            ...
            catch (Exception ex)
            {
                ...
                {
                    Console.WriteLine($@"End Time: {DateTime.UtcNow.ToLongTimeString()}");
