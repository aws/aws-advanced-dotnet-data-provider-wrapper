...
public class AuroraTestUtils
{
    ...
    public DBInstance? GetRdsInstanceInfoByArn(string instanceArn)
    {
        ...
        return response.Result.DBInstances is { Count: > 0 } ? response.Result.DBInstances.First() : null;
    }
    ...
    {
        return this.GetAuroraInstanceIds(
    ...
    public string CreateSecrets(string secretsName)
    {
        ...
        {
            // Secret exists but is scheduled for deletion. Restore it and update the value.
            var restoreRequest = new RestoreSecretRequest { SecretId = secretsName };
            this.secretsManagerClient.RestoreSecretAsync(restoreRequest).GetAwaiter().GetResult();
            ...
            var updateResponse = this.secretsManagerClient.PutSecretValueAsync(updateRequest).GetAwaiter().GetResult();
